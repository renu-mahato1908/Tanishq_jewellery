
import Col from 'react-bootstrap/Col';
import Container from 'react-bootstrap/Container';
import Image from 'react-bootstrap/Image';
import Row from 'react-bootstrap/Row';
import './Component.css';
import { FiHeart } from "react-icons/fi";
import { HiOutlineShoppingCart } from "react-icons/hi";



const Gold = () => {
  return (
    <div>


      <section>
        <Container>
          <Row>



            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dwe4bf267c/images/hi-res/510122FAAAA00_1.jpg?sw=480&sh=480' thumbnail />
                <div className="details">
                  <p> Gold Leaf Patterned Finger Ring</p>
                  <h5>₹56668</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>
            </Col>


            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw57f5c07f/images/hi-res/504025HMTAAA00_1.jpg?sw=480&sh=480" thumbnail />
                <div className="details">
                  <p>Enticing Petite Drop Earrings</p>
                  <h5>₹ 55283.</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>
            </Col>


            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw2b44b79c/images/hi-res/515090DBQAGA00_2.jpg?sw=480&sh=480" thumbnail />
                <div className="details">
                  <p>Leaf Motif Gold Drop Earrings</p>
                  <h5>₹ 53936</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>
            </Col>



          </Row>

          <Row>
            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw4ec4effa/images/hi-res/51D3D1DMQABA00_2.jpg?sw=480&sh=480' thumbnail />
                <div className="details">
                  <p>Stunning Abstract Drop Earrings</p>
                  <h5>₹ 69379</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>
            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dwa73dc74e/images/hi-res/51D3D1HCOABA00_2.jpg?sw=480&sh=480' thumbnail />
                <div className="details">
                  <p>Netted Miracles Hoop Earrings</p>
                  <h5>₹ 47693</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>

            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw3b97d2e8/images/hi-res/513218PQEAAA00_1.jpg?sw=480&sh=480' thumbnail />
                <div className="details">

                  <p>Minimalistic Leaf Motif Gold Pendant</p>
                  <h5>₹ 28962</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>
            </Col>
          </Row>

          <Row>
            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw270ff516/images/hi-res/51D2D1PQVAAA00_2.jpg?sw=480&sh=480' thumbnail />
                <div className="details">

                  <p>Ethinic Elegant Pendant 12k gold </p>
                  <h5> ₹ 36499</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>
            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>



                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw54133b13/images/hi-res/510895FLAAA00_1.jpg?sw=480&sh=480' thumbnail />
                <div className="details">

                  <p> 22 Karat Yellow Gold Finger Ring</p>
                  <h5>₹ 29247</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw1d3943da/images/hi-res/510783FMAAB00_2.jpg?sw=480&sh=480' thumbnail />
                <div className="details">
                  <p>Delecate Contemporary Gold Ring</p>
                  <h5> ₹ 23770</h5>
                </div>
                <button className='buy-btn'>Buy now</button>

              </div>
            </Col>
          </Row>

          <Row >
            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw85ff0047/images/hi-res/513515YQKAA00_1.jpg?sw=640&sh=640' thumbnail />
                <div className="details">
                  <p>Charming Gold Beaded Mangalsutra</p>
                  <h5>₹1 24 327</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw6670f257/images/hi-res/513018BTSAA00_1.jpg?sw=480&sh=480" alt='' thumbnail />
                <div className="details">
                  <p>Dual Toned Gold Bracelet</p>
                  <h5>₹1 93679</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>


            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw40d3e453/images/hi-res/512220BANAA00_1.jpg?sw=640&sh=640' thumbnail />
                <div className="details">
                  <p>Wavy Gold Bracelet</p>
                  <h5>₹106082</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>




          </Row>
        </Container>

      </section>

      <section>
        <Container>
          <Row>
            <Col>
              <h2>Men's Jewellery section</h2>
            </Col>
          </Row>

          <Row >
            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw6b9eca96/images/hi-res/51G5C1FHXAA00_1.jpg?sw=640&sh=640' roundedCircle/>
                <div className="details">
                  <p>Radiant Cross Gold Ring For Men</p>
                  <h5>₹155501</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dwec7e1635/images/hi-res/51G5C1PHMAAA00_1.jpg?sw=640&sh=640" alt='' roundedCircle />
                <div className="details">
                  <p>Hexagonal Star Gold Pendant For Men</p>
                  <h5>₹74163</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>


            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dwda72e922/images/hi-res/50G5C1CBVAA00_1.jpg?sw=640&sh=640' roundedCircle />
                <div className="details">
                  <p>Rope Twist Gold Chain For Men</p>
                  <h5>₹169443</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>




          </Row>


          <Row >
            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src='https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw7d04a14e/images/hi-res/51G5C1FHRAA00_2.jpg?sw=480&sh=480' roundedCircle />
                <div className="details">
                  <p>Heritage Edge gold Rings for men</p>
                  <h5>₹ 69293</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dwcd658d59/images/hi-res/51F2D1VAF1A00_2.jpg?sw=640&sh=640" alt='' roundedCircle />
                <div className="details">
                  <p>
                    Timeless Glow Gold Bangle for Men</p>
                  <h5>₹51747</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>


            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dwf4f1d3c8/images/hi-res/51D5PTPODAAA00_1.jpg?sw=640&sh=640" roundedCircle />
                <div className="details">
                  <p>Veer Hanuman Gold Pendant for Men</p>
                  <h5>₹ 30390</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>




          </Row>

          <Row >
            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dwcd2d29a5/images/hi-res/HLS2I2FMFAV87_2.jpg?sw=480&sh=480"  roundedCircle />
                <div className="details">
                  <p>Fashionable Men's Gold Finger Ring</p>
                  <h5>₹ 93052</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>

            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw81228298/images/hi-res/50G5M2CCCAA00_2.jpg?sw=480&sh=480" alt='' roundedCircle/>
                <div className="details">
                  <p>
                    Majestic Gold Chain for Men</p>
                  <h5>₹151010</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>


            <Col md={4}>
              <div className="polaroid">
                <button className='wishlist-btn'><FiHeart className="wishlist-icon" /></button>
                <button className='addtocart-btn'><HiOutlineShoppingCart className='add-icon' /></button>


                <Image src="https://www.tanishq.co.in/dw/image/v2/BKCK_PRD/on/demandware.static/-/Sites-Tanishq-product-catalog/default/dw1e94f491/images/hi-res/51G5C1PHOAAA00_1.jpg?sw=640&sh=640" roundedCircle />
                <div className="details">
                  <p>Spiral Bullet Gold Pendant For Men</p>
                  <h5>₹ 53914</h5>
                </div>
                <button className='buy-btn'>Buy now</button>
              </div>
            </Col>




          </Row>



        </Container>
      </section>
    </div>
  )
}

export default Gold
